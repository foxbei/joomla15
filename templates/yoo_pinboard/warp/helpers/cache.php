...
class WarpCache extends WarpObject {
	...
	function __construct($options = array()) {
		...
		$this->options  = array_merge(array('gzip' => false, 'data_uri' => false), $options);
...
class WarpCacheStylesheets extends WarpCache {
	...
	function _buildCache($files, $cache_file) {
		...
		foreach ($files as $file => $filepath) {
			...
			WarpCacheStylesheets::_buildPath(null, dirname($file).'/', ($this->options['data_uri'] ? dirname($filepath) : null));
	...
	function _buildPath($matches, $base = null, $filepath = null) {
		...
		if ($_filepath != null && preg_match('/\.(gif|png|jpg)$/i', $matches[1], $extension)) {
			
			$image = realpath(rtrim($_filepath, '/').'/'.$matches[1]);
			
			 // check if image exists and filesize < 30kb (IE8 limit)
			if (file_exists($image) && filesize($image) <= 30720) {
				return sprintf('url(data:image/%s;base64,%s)', str_replace('jpg', 'jpeg', strtolower($extension[1])), base64_encode(file_get_contents($image)));
			}		
		}
